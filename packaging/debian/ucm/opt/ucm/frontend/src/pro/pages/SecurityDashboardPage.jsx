...
export default function SecurityDashboardPage() {
  ...
  const handleRotateSecrets = async () => {
    ...
    try {
      ...
      showSuccess('New secret generated - follow instructions to apply')
      
      // Show instructions in alert
      if (data.instructions) {
        alert(`Rotation Instructions:\n\n${data.instructions.join('\n')}`)
      }
    } catch (error) {
      showError(error.message || 'Failed to rotate secrets')
    } finally {
      setRotating(false)
    }
  }
  ...
  const handleVerifyAudit = async () => {
    ...
    try {
      ...
      if (data.valid) {
        showSuccess(`Audit integrity verified (${data.checked} entries)`)
      } else {
        showError('Audit integrity check failed!')
      }
    } catch (error) {
      showError(error.message || 'Failed to verify audit')
    } finally {
      setVerifying(false)
    }
  }
  ...
  const stats = [
    {
      ...
      value: secretsStatus?.jwt_secret?.configured ? 'Configured' : 'Not Set',
      icon: Key,
    ...
    },
    {
      ...
      value: auditVerify?.valid ? 'Verified' : 'Not Checked',
      icon: ShieldCheck,
  ...
  return (
    <ResponsiveLayout
      ...
      subtitle="Monitor security status and manage secrets"
      icon={ShieldCheck}
      stats={stats}
    ...
    >
      <div className="space-y-6 p-4">
              ...
              <CompactGrid cols={3}>
                <div className="p-3 rounded-lg bg-bg-secondary">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-text-primary">JWT Secret</span>
                  </div>
                  <div className="text-sm text-text-secondary">
                    {secretsStatus.jwt_secret?.configured ? 'Configured' : 'Not configured'}
                  </div>
                ...
                <div className="p-3 rounded-lg bg-bg-secondary">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-text-primary">Session Secret</span>
                  </div>
                  <div className="text-sm text-text-secondary">
                    {secretsStatus.session_secret?.configured ? 'Configured' : 'Not configured'}
                  </div>
                </div>
                ...
                <div className="p-3 rounded-lg bg-bg-secondary">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="font-medium text-text-primary">Encryption Key</span>
                  </div>
                  <div className="text-sm text-text-secondary">
                    {secretsStatus.encryption_key?.configured ? 'Configured' : 'Not set (keys unencrypted)'}
                  </div>
                </div>
              </CompactGrid>
        ...
        <CompactSection 
          ...
          badge={anomalies.length > 0 ? `${anomalies.length} alerts` : null}
        ...
        >
          {anomalies.length === 0 ? (
            <div className="flex items-center gap-3 p-4 rounded-lg bg-status-success/10">
              ...
              <div>
                <div className="font-medium text-text-primary">No anomalies detected</div>
                <div className="text-sm text-text-secondary">Last 24 hours activity looks normal</div>
              </div>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-text-primary">
                      {anomaly.details?.type || 'Unknown anomaly'}
                    </div>
                    <div className="text-sm text-text-secondary">
        ...
        >
          <div className="space-y-4">
            <p className="text-sm text-text-secondary">
              Audit logs use a cryptographic hash chain. Each entry contains a hash of the previous entry,
              making it impossible to modify or delete logs without detection.
            </p>
            ...
            {auditVerify && (
              <div className={cn(
                ...
                <div>
                  <div className="font-medium text-text-primary">
                    {auditVerify.valid ? 'Integrity Verified' : 'Integrity Check Failed'}
                  </div>
                  <div className="text-sm text-text-secondary">
                    {auditVerify.checked} entries checked
                    {auditVerify.errors?.length > 0 && ` - ${auditVerify.errors.length} errors found`}
                  </div>
                </div>
              </div>
            )}
