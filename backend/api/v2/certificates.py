...
def get_certificate(cert_id):
    ...
    data['extensions'] = _parse_certificate_extensions(cert.crt)
...
def _parse_certificate_extensions(pem_data):
    ...
    try:
        ...
        # If not raw PEM, it's base64-encoded PEM — decode first
    ...
    if ext:
        ...
        for attr, label in [
            ...
            try:
                ...
                pass
        # encipher_only and decipher_only only valid when key_agreement is set
        try:
            if ku.key_agreement:
                if ku.encipher_only:
    ...
    # Subject Alternative Names (full parsing including UPN/otherName)
    ...
    if ext:
        ...
        for name in san:
            if isinstance(name, x509.DNSName):
                ...
                san_entries.append({'type': 'DirName', 'value': ', '.join(dn_parts)})
            elif isinstance(name, x509.OtherName):
                # UPN (User Principal Name) — OID 1.3.6.1.4.1.311.20.2.3
                oid_str = name.type_id.dotted_string
                try:
                    # UPN is encoded as UTF8String inside the OtherName value
                    from asn1crypto.core import UTF8String
                    upn_value = UTF8String.load(name.value).native
                    if oid_str == '1.3.6.1.4.1.311.20.2.3':
                        san_entries.append({'type': 'UPN', 'value': upn_value})
                    else:
                        san_entries.append({'type': f'OtherName({oid_str})', 'value': upn_value})
                except Exception:
                    san_entries.append({'type': f'OtherName({oid_str})', 'value': name.value.hex()})
            elif isinstance(name, x509.RegisteredID):
                san_entries.append({'type': 'RegisteredID', 'value': name.value.dotted_string})
            else:
                san_entries.append({'type': 'Unknown', 'value': str(name)})
